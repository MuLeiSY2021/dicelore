...
**Goal:** 把项目对外定位从「安价 GM 框架」升维为「agentic 时代的角色扮演宿主」——先改 wiki 01 上游口径 + 新增 ADR-0021，再下推 README 九段重写。

**Architecture:** 纯文档/定位重构。单向推导：wiki 01（问题域升维 + 安价两页措辞微调）+ ADR-0021 为上游，README 为下游。下游引上游口径，不反向。技术设计层（02/03/04）一行不动。
...
```markdown
；**2026-06-22 §一句话升维**——命题由「安价 GM 框架」上提为「agentic 时代的角色扮演宿主」，安价重述为首发载体，详见 [ADR-0021](../05-决策记录-ADR/README.md)）。
```
...
### Task 3: 新增 ADR-0021（定位升维 + 不商业化）

**Files:**
- Modify: `docs/wiki/05-决策记录-ADR/README.md`（在 ADR-0019 之后追加 ADR-0021）

**Interfaces:**
- Consumes: Task 1 口径。
- Produces: ADR-0021，被 Task 1/2/4 引用。

- [ ] **Step 1: 追加 ADR-0021（逐字成稿）**

在文件末尾「待决策」段之前（或最末一条 ADR 之后）追加：

```markdown
## ADR-0021 定位升维：从「安价 GM 框架」到「agentic 时代的角色扮演宿主」
...
核对：① ADR-0021 三段式齐全（背景/决策/后果）；② 含「不商业化/纯 AGPL」「独立起源」「攻击范式不攻击产品」「被否：取代/接棒」；③ 编号范围已同步；④ 链接有效。

- [ ] **Step 4: Commit**

```bash
git add docs/wiki/05-决策记录-ADR/README.md docs/wiki/README.md
git commit -m "docs(adr): 新增 ADR-0021 定位升维 + 放弃商业化
...
**Interfaces:**
- Consumes: Task 1 定位口径、Task 3 ADR-0021、Global Constraints 的 slogan。
- Produces: 对外门面。
...
要点（散文，守叙事宪法）：① 玫瑰叹息——AI 有求必应→虚拟太完美→失真无聊；② `prompt-based → agentic` 范式换代：提示词范式状态住在 context 里、有求必应是结构必然；③ 大方致谢酒馆是把提示词范式做到极致的灵感来源；④ 诚实划界：不抢纯陪伴/拟真，只打「机制可信/有对抗」的赛道；⑤ **起源故事（作者自述，必含、第一人称口吻）**：「虚拟太完美了，像一朵没有味道也没有刺的玫瑰」这句话是**作者的一位朋友**说的，作者被它**狠狠打动**，于是发起这个开源项目、致力于解决这个问题——因为作者相信 **agent 架构**正是能把「刺」装回去的那把钥匙。**禁止**：贬低酒馆产品、喊杀手/接棒、讲 fork 传承史。
...
对照 spec §8 全部 7 条验收逐条打勾；重点确认 README 定位口径与 wiki 01/ADR-0021 不矛盾（单向推导）。
...
- **Wave 1（并发）**：Task 1（问题域升维）· Task 2（安价两页）· Task 3（ADR-0021）——三者改不同文件、口径已在 spec 定死，可并发派 subagent。
...
- **Placeholder scan**：slogan/对比表/ADR-0021/许可证删改/hero 均逐字给出；叙述段(T4S2/S4)给要点+禁止项(文档任务散文的合理粒度，非占位)。
